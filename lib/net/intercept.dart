...
class AuthInterceptor extends Interceptor {
  ...
  void onRequest(RequestOptions options, RequestInterceptorHandler handler) {
    ...
    // 仅云服务请求添加 Authorization 头；主设备（直连PC）不需要此头
    // 对齐 smdcapp：PCTableHttpUtil 的 OkHttpClient 无 Authorization 拦截器
    final String path = options.path;
    final bool isCloudRequest = path.startsWith(HttpApi.baseUrlYtt) ||
        !path.contains('/api/');
    if (accessToken.isNotEmpty && isCloudRequest) {
...
class AdapterInterceptor extends Interceptor{
  ...
  Response<dynamic> adapterData(Response<dynamic> response) {
    ...
    if (response.statusCode == ExceptionHandle.success || response.statusCode == ExceptionHandle.success_not_content) {
      // 透传原始响应数据，不做格式转换，由 request() 直接处理 retcode/retmsg
      if (content.isEmpty) {
